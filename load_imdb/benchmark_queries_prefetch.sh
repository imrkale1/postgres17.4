...
RESULTS_DIR="benchmark_results_prefetch"
...
echo "Testing: btree_leaf_prefetch = off vs on" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
echo "================================================" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
...
echo "=== BIG LOOP 1: btree_leaf_prefetch = ON first, then OFF ===" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
echo "" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
...
for sql_file in $SQL_FILES; do
    ...
    # Test with btree_leaf_prefetch = ON
    echo "  Testing with btree_leaf_prefetch = ON..." | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
    ...
    # Test with btree_leaf_prefetch = OFF
    echo "  Testing with btree_leaf_prefetch = OFF..." | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
    START_TIME=$(date +%s%3N)
    START_READABLE=$(date '+%Y-%m-%d %H:%M:%S.%3N')
...
echo "=== BIG LOOP 2: btree_leaf_prefetch = OFF first, then ON ===" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
echo "" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
...
for sql_file in $SQL_FILES; do
    ...
    echo "  btree_leaf_prefetch = OFF: ${AVG_OFF}ms (avg: ${OFF_DURATIONS_ROUND1["$sql_file"]}ms, ${OFF_DURATIONS_ROUND2["$sql_file"]}ms)" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
    echo "  btree_leaf_prefetch = ON:  ${AVG_ON}ms (avg: ${ON_DURATIONS_ROUND1["$sql_file"]}ms, ${ON_DURATIONS_ROUND2["$sql_file"]}ms)" | tee -a "$RESULTS_DIR/$OUTPUT_FILE"
...
tail -n +2 "$RESULTS_DIR/$COMPARISON_FILE" | awk -F',' '
    ...
    END {
        if (count > 0) {
            ...
            printf "btree_leaf_prefetch = on faster: %d (%.1f%%)\n", on_faster, (on_faster/count)*100
            printf "btree_leaf_prefetch = off faster: %d (%.1f%%)\n", off_faster, (off_faster/count)*100
